...
const Dashboard = () => {
  ...
  const myEntry = (user && leaderboard.find(l => 
    l.studentName?.trim().toLowerCase() === user.fullName?.trim().toLowerCase()
  )) || { quizzesAttended: 0, averageScore: 0 };
  const studentStats = [
          ...
          {leaderboard.length === 0 ? (
            ...
            <div className="table-container">
              <table className="table">
                ...
                <tbody>
                  {leaderboard.map((l, i) => (
                    <tr key={i} style={{ background: (user && l.studentName?.trim().toLowerCase() === user.fullName?.trim().toLowerCase()) ? 'var(--accent-light)' : 'transparent' }}>
                      <td style={{ fontWeight: 900, fontSize: '1rem' }}>#{i + 1}</td>
                      <td>
                        <span style={{ fontWeight: (user && l.studentName?.trim().toLowerCase() === user.fullName?.trim().toLowerCase()) ? 800 : 500 }}>
                          {l.studentName} {(user && l.studentName?.trim().toLowerCase() === user.fullName?.trim().toLowerCase()) && <span className="badge badge-accent" style={{ marginLeft: '8px', padding: '2px 8px', fontSize: '0.6rem' }}>YOU</span>}
                        </span>
                      </td>
