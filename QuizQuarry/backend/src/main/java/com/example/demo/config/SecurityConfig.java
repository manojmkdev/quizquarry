        ...
        return http.csrf(csrf -> csrf.disable())
                ...
                .oauth2Login(oauth -> oauth
                        .successHandler((request, response, authentication) -> {
                            ...
                            if (name == null) {
                                name = oauthUser.getAttribute("login"); // Fallback for GitHub
                            }
                            if (name == null) {
                                name = email; // Fallback to email if no name is available
                            }
                            ...
                            response.sendRedirect("http://localhost:3000/oauth2/callback?token=" + token + "&fullName=" + java.net.URLEncoder.encode(user.getFullName(), "UTF-8") + "&role=" + user.getRole().name());
                        }))
