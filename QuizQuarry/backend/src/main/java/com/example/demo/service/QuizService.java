...
public class QuizService {
    ...
    public List<QuizDto> getQuizzes(String email) {
        ...
        return qRepo.findAll().stream()
                .filter(q -> user != null && (
                    (user.getRole() == SystemUser.Role.INSTRUCTOR && q.getInstructor() != null && q.getInstructor().getId().equals(user.getId()))
                    || (user.getRole() == SystemUser.Role.STUDENT && q.getStatus() == QuizAssessment.Status.PUBLISHED)
                ))
                .map(q -> {
